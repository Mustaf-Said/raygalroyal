...
export default function Pricing() {
  ...
  const supportPlan = { key: "support", icon: Check, color: "emerald" }
  const paidPlans = [
  ...
  const renderCard = (plan: typeof supportPlan & { popular?: boolean }, index: number) => {
        ...
        <div className="flex flex-col mb-8">
          {isSupportPlan ? (
            ...
            <div className="min-h-[64px]">
              {discountActive && (
                <p className="text-gray-500 line-through text-sm animate-fade-in">
            ...
            <>
              ...
              {discountActive && (
                <span className="inline-block bg-green-500/10 text-green-400 text-xs font-semibold px-3 py-1 rounded-full mt-2 animate-fade-in">
  ...
  return (
    <section id="pricing" className="py-24 bg-white dark:bg-gray-950 overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Support card — rendered before the toggle, never affected by discount */}
        <div className="max-w-sm mx-auto mb-14">
          {renderCard(supportPlan, 0)}
        </div>

        {/* Discount toggle — only above Standard, Professional, Enterprise */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
          {paidPlans.map((plan, index) => renderCard(plan, index + 1))}
        </div>
      </div>
    </section>
  )
