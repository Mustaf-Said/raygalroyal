      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-20">
          <h2 className="text-4xl md:text-5xl font-black text-gray-900 dark:text-white mb-6">
            {t.paymentIntegration.title}
          </h2>
          <p className="text-xl text-gray-600 dark:text-gray-400">
            {t.paymentIntegration.subtitle}
          </p>
        </div>
          ...
          >
            ...
            <div className="space-y-6">
              <div className="p-6 bg-gray-50 dark:bg-gray-900 rounded-2xl">
                <div className="text-sm font-bold text-gray-400 uppercase mb-4">{t.paymentIntegration.cardDetails}</div>
              ...
              <button className="w-full py-4 bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">
                {t.paymentIntegration.payWithStripe}
              </button>
              
              <div className="flex items-center justify-center gap-2 text-xs text-gray-400">
                <ShieldCheck className="w-4 h-4" />
                {t.paymentIntegration.secureStripe.toUpperCase()}
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="space-y-6">
              <div className="p-6 bg-blue-50 dark:bg-blue-900/10 rounded-2xl text-center">
                <p className="text-blue-600 dark:text-blue-400 font-medium mb-2">{t.paymentIntegration.oneClickCheckout}</p>
                <div className="text-sm text-gray-400">{t.paymentIntegration.paypalDescription}</div>
              </div>
              ...
              <div className="flex items-center justify-center gap-2 text-xs text-gray-400">
                <ShieldCheck className="w-4 h-4" />
                {t.paymentIntegration.protectedByPaypal.toUpperCase()}
              </div>
            </div>
          </motion.div>
        </div>
